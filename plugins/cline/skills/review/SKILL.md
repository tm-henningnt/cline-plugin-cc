...
2. Pipe the relevant diff into the dispatcher:
   ...
   ```bash
   git diff <base> | CLINE_PLUGIN_HOST=codex node "${PLUGIN_ROOT}/scripts/dispatcher.mjs" review "$ARGUMENTS"
   ```

   When no base ref is available, run `git diff | CLINE_PLUGIN_HOST=codex node "${PLUGIN_ROOT}/scripts/dispatcher.mjs"
